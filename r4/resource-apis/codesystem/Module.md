# FHIR CodeSystem API Template

This Ballerina-based template implementation provides a RESTful FHIR API for managing `CodeSystem` resources, adhering to the FHIR R4 specification. The API is designed to support CRUD operations International FHIR R4 CodeSystem FHIR API.

| Module/Element       | Version |
| -------------------- | ------- |
| FHIR version         | r4 |
| Implementation Guide | [http://hl7.org/fhir](http://hl7.org/fhir) |
| Profile URL          |[http://hl7.org/fhir/StructureDefinition/shareablecodesystem](http://hl7.org/fhir/StructureDefinition/shareablecodesystem)|
...
```sh
bal new -t healthcare/health.fhir.r4.international401.codesystem CodeSystemAPI
```
...
```sh
curl http://localhost:9090/fhir/r4/CodeSystem
```
...
```http
GET /fhir/r4/CodeSystem/{id}
```
- **Response:** Returns a `CodeSystem` resource or an `OperationOutcome` if not found.

```http
GET /fhir/r4/CodeSystem/{id}/_history/{vid}
```
- **Response:** Returns a specific historical version of a `CodeSystem` resource.

```http
GET /fhir/r4/CodeSystem
```
- **Response:** Returns a `Bundle` containing matching `CodeSystem` resources.

```http
POST /fhir/r4/CodeSystem
```
- **Request Body:** `CodeSystem` resource.
- **Response:** Returns the created `CodeSystem` resource.

```http
PUT /fhir/r4/CodeSystem/{id}
```
- **Request Body:** Updated `CodeSystem` resource.
- **Response:** Returns the updated `CodeSystem` resource.

```http
PATCH /fhir/r4/CodeSystem/{id}
```
- **Request Body:** JSON Patch object.
- **Response:** Returns the updated `CodeSystem` resource.

```http
DELETE /fhir/r4/CodeSystem/{id}
```
- **Response:** Returns an `OperationOutcome` indicating success or failure.

```http
GET /fhir/r4/CodeSystem/{id}/_history
```
- **Response:** Returns a `Bundle` of historical versions of the `CodeSystem` resource.

```http
GET /fhir/r4/CodeSystem/_history
```
- **Response:** Returns a `Bundle` of historical versions of all `CodeSystem` resources.
...
```json
  ...
  ```typescript
    isolated resource function post fhir/r4/CodeSystem/\$custom\-operation(r4:FHIRContext fhirContext, CodeSystem procedure) returns r4:OperationOutcome|r4:FHIRError {
...
- Add the profile type to the aggregated resource type.
  - **Example:** `public type CodeSystem r4:CodeSystem|<Other_CodeSystem_Profile>;`
- Add the new profile URL in `api_config.bal` file. You need to add it as a string inside the `profiles` array.
  - **Example:**
    ```ballerina
    profiles: ["http://hl7.org/fhir/StructureDefinition/CodeSystem", "new_profile_url"]
    ```
