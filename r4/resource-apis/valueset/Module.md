# FHIR ValueSet API Template

This Ballerina-based template implementation provides a RESTful FHIR API for managing `ValueSet` resources, adhering to the FHIR R4 specification. The API is designed to support CRUD operations International FHIR R4 ValueSet FHIR API.

| Module/Element       | Version |
| -------------------- | ------- |
| FHIR version         | r4 |
| Implementation Guide | [http://hl7.org/fhir](http://hl7.org/fhir) |
| Profile URL          |[http://hl7.org/fhir/StructureDefinition/shareablevalueset](http://hl7.org/fhir/StructureDefinition/shareablevalueset)|
...
```sh
bal new -t healthcare/health.fhir.r4.international401.valueset ValueSetAPI
```
...
```sh
curl http://localhost:9090/fhir/r4/ValueSet
```
...
```http
GET /fhir/r4/ValueSet/{id}
```
- **Response:** Returns a `ValueSet` resource or an `OperationOutcome` if not found.

```http
GET /fhir/r4/ValueSet/{id}/_history/{vid}
```
- **Response:** Returns a specific historical version of a `ValueSet` resource.

```http
GET /fhir/r4/ValueSet
```
- **Response:** Returns a `Bundle` containing matching `ValueSet` resources.

```http
POST /fhir/r4/ValueSet
```
- **Request Body:** `ValueSet` resource.
- **Response:** Returns the created `ValueSet` resource.

```http
PUT /fhir/r4/ValueSet/{id}
```
- **Request Body:** Updated `ValueSet` resource.
- **Response:** Returns the updated `ValueSet` resource.

```http
PATCH /fhir/r4/ValueSet/{id}
```
- **Request Body:** JSON Patch object.
- **Response:** Returns the updated `ValueSet` resource.

```http
DELETE /fhir/r4/ValueSet/{id}
```
- **Response:** Returns an `OperationOutcome` indicating success or failure.

```http
GET /fhir/r4/ValueSet/{id}/_history
```
- **Response:** Returns a `Bundle` of historical versions of the `ValueSet` resource.

```http
GET /fhir/r4/ValueSet/_history
```
- **Response:** Returns a `Bundle` of historical versions of all `ValueSet` resources.
...
```json
  ...
  ```typescript
    isolated resource function post fhir/r4/ValueSet/\$custom\-operation(r4:FHIRContext fhirContext, ValueSet procedure) returns r4:OperationOutcome|r4:FHIRError {
...
- Add the profile type to the aggregated resource type.
  - **Example:** `public type ValueSet r4:ValueSet|<Other_ValueSet_Profile>;`
- Add the new profile URL in `api_config.bal` file. You need to add it as a string inside the `profiles` array.
  - **Example:**
    ```ballerina
    profiles: ["http://hl7.org/fhir/StructureDefinition/ValueSet", "new_profile_url"]
    ```
