# FHIR Binary API Template

This Ballerina-based template implementation provides a RESTful FHIR API for managing `Binary` resources, adhering to the FHIR R4 specification. The API is designed to support CRUD operations International FHIR R4 Binary FHIR API.

| Module/Element       | Version |
| -------------------- | ------- |
| FHIR version         | r4 |
| Implementation Guide | [http://hl7.org/fhir](http://hl7.org/fhir) |
| Profile URL          |[http://hl7.org/fhir/StructureDefinition/Binary](http://hl7.org/fhir/StructureDefinition/Binary)|
...
```sh
bal new -t healthcare/health.fhir.r4.international401.binary BinaryAPI
```
...
```sh
curl http://localhost:9090/fhir/r4/Binary
```
...
```
- **Response:** Returns a `Binary` resource or an `OperationOutcome` if not found.

```http
GET /fhir/r4/Binary/{id}/_history/{vid}
```
- **Response:** Returns a specific historical version of a `Binary` resource.

```http
GET /fhir/r4/Binary
```
- **Response:** Returns a `Bundle` containing matching `Binary` resources.

```http
POST /fhir/r4/Binary
```
- **Request Body:** `Binary` resource.
- **Response:** Returns the created `Binary` resource.

```http
PUT /fhir/r4/Binary/{id}
```
- **Request Body:** Updated `Binary` resource.
- **Response:** Returns the updated `Binary` resource.

```http
PATCH /fhir/r4/Binary/{id}
```
- **Request Body:** JSON Patch object.
- **Response:** Returns the updated `Binary` resource.

```http
DELETE /fhir/r4/Binary/{id}
```
- **Response:** Returns an `OperationOutcome` indicating success or failure.

```http
GET /fhir/r4/Binary/{id}/_history
```
- **Response:** Returns a `Bundle` of historical versions of the `Binary` resource.

```http
GET /fhir/r4/Binary/_history
```
- **Response:** Returns a `Bundle` of historical versions of all `Binary` resources.
...
```json
  ...
  ```typescript
    isolated resource function post fhir/r4/Binary/\$custom\-operation(r4:FHIRContext fhirContext, Binary procedure) returns r4:OperationOutcome|r4:FHIRError {
...
- Add the profile type to the aggregated resource type.
  - **Example:** `public type Binary r4:Binary|<Other_Binary_Profile>;`
- Add the new profile URL in `api_config.bal` file. You need to add it as a string inside the `profiles` array.
  - **Example:**
    ```ballerina
    profiles: ["http://hl7.org/fhir/StructureDefinition/Binary", "new_profile_url"]
    ```
